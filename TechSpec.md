...
- lives: int - Number of hearts remaining for the player (default: 3).
...
- gameStartTime: Date - Timestamp when the first tile is revealed.
- minimapExpanded: boolean - Tracks if minimap is in expanded state.

Constants

- TILE_SIZE: int - Size of each tile in pixels (32).
- MAP_ROWS: int - Number of rows in the game map (40).
- MAP_COLS: int - Number of columns in the game map (40).
- VIEWPORT_ROWS: int - Number of rows visible on screen (12).
- VIEWPORT_COLS: int - Number of columns visible on screen (20).
- PLAYER_SPEED: int - Player movement speed in pixels per frame (4).
- CAMERA_LERP: float - Camera smoothing factor (0.1).
- MINIMAP_SIZE: int - Size of the minimap in pixels (150).
...
lerp(start: float, end: float, t: float)

- Behavior: Performs linear interpolation between start and end values for smooth camera movement.
...
- firstReveal: boolean - Tracks if this is the player's first tile reveal.
...
- Automatically places a flag on revealed mine squares.
- Recursively reveals adjacent squares if the current square has no adjacent mines.

flagSquare(x: int, y: int)

- Behavior: Flags or unflags the specified square.

ensureSafeFirstReveal(revealX: int, revealY: int)

- Behavior: Ensures the first revealed area is safe by relocating any mines in the 3x3 area around the reveal point.

countWaterTilesAround(x: int, y: int)

- Behavior: Counts the number of water tiles in the 8 squares surrounding the given coordinates.

countFlagsAround(x: int, y: int)

- Behavior: Counts the number of flags in the 8 squares surrounding the given coordinates.

chordTiles(x: int, y: int)

- Behavior: If the number of flags around a revealed tile matches its number, reveals all unflagged adjacent tiles.
...
- position: Point - Current position of the player (x, y in pixels).
...
- Behavior: Updates the player's position based on input (Arrow keys). Prevents movement onto water cells.
- Includes bounds checking to keep player within map boundaries.
- Updates camera position to follow player with smooth lerp movement.
...
- Behavior: Reveals or flags the tile in front of the player based on input (Enter for reveal, Space for flag).
...
- heartsDisplay: HTMLElement - Shows the number of hearts left using heart shapes.
...
- Behavior: Toggles between small and expanded minimap views (M key).
- Small view shows minimap in bottom right corner.
- Expanded view shows minimap covering entire canvas.

drawFlag(x: int, y: int)

- Behavior: Draws a flag icon at the specified coordinates using a red flag on a gray pole.

renderMinimap()

- Behavior: Renders the minimap with different colors for revealed tiles (brown), water (red), flags (yellow), and unrevealed tiles (green).
- Shows player position as a blue square.
